...
#' @description Computes principal components of a distance matrix at multiple distance thresholds to generate multi-scale spatial predictors. Each distance threshold defines a different neighborhood scale, and PCA is applied to the weighted distance matrix at each scale.
...
#' @param max.spatial.predictors Integer specifying the maximum number of spatial predictors to retain. If the total number of generated predictors exceeds this value, only the first `max.spatial.predictors` are kept (ordered by variance explained). Default: `NULL` (keeps all predictors).
...
#'
#' Different distance thresholds capture spatial patterns at different scales. Combining predictors from multiple thresholds allows models to account for spatial autocorrelation operating at multiple spatial scales simultaneously. This is analogous to [mem_multithreshold()] but uses PCA instead of Moran's Eigenvector Maps.
#'
