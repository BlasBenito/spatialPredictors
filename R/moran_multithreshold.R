...
#'
#' The plot shows Moran's I values across distance thresholds. Peaks in Moran's I indicate spatial scales where autocorrelation is strongest. The `max.moran` and `max.moran.distance.threshold` values identify the optimal scale.
#' @seealso [moran()], [mem_multithreshold()], [default_distance_thresholds()]
#' @examples
#' set.seed(1)
